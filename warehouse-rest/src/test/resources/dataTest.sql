...
insert into USER_ROL (id, user_id, rol_id) values (REPLACE(random_uuid(), '-', ''), SELECT id FROM USERS WHERE username = 'Admon', SELECT id FROM ROL WHERE rol = 'ROLE_ADMIN');
insert into USER_ROL (id, user_id, rol_id) values (REPLACE(random_uuid(), '-', ''), SELECT id FROM USERS WHERE username = 'Auditor', SELECT id FROM ROL WHERE rol = 'ROLE_AUDITOR');